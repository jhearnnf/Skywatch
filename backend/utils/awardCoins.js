 ...
 * This is the sum of all airstarsToNextLevel values (levels 1–9).
...
async function getCycleThreshold() {
  ...
  for (const lv of levels) {
    if (lv.airstarsToNextLevel != null) total += lv.airstarsToNextLevel;
  }
  return total || CYCLE_THRESHOLD;
}

/**
 * Award airstars to a user, handling:
 *   - totalAirstars (always grows — used for leaderboard)
 *   - cycleAirstars (resets on rank promotion, drives level display)
...
async function awardCoins(userId, amount, reason, label, briefId = null) {
  ...
  const newTotal = incremented.totalAirstars;
  let   finalCycle      = incremented.cycleAirstars;
  ...
  if (finalCycle >= cycleThreshold) {
    ...
    const cycleAdjustment = finalCycle - incremented.cycleAirstars;
    ...
    // must NOT silently leave cycleAirstars inconsistent. Reload, recompute the
    ...
    if (!guarded) {
      ...
      if (reloaded) {
        // The other writer applied amount via $inc too, so cycleAirstars now
        ...
        // cycleAirstars should always be in [0, threshold). Take the existing
        ...
        const currentCycle = reloaded.cycleAirstars ?? 0;
        const correctedCycle = currentCycle % cycleThreshold;
        if (correctedCycle !== currentCycle) {
          await User.findByIdAndUpdate(userId, { $set: { cycleAirstars: correctedCycle } });
        }
      }
    }
  }
