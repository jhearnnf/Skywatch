...
describe('normaliseLoudness', () => {
  ...
  it('strips the container metadata on the way out', async () => {
    queue({ stderr: LOUDNORM_JSON }, { code: 0 });

    await audio.normaliseLoudness('raw.mp4', 'out.mp4');

    // Remotion's "Made with Remotion" comment and ffmpeg's own encoder tag are
    // a plain-text "generated by software" stamp; neither may reach a platform.
    const applied = spawn.mock.calls[1][1].join(' ');
    expect(applied).toContain('-map_metadata -1');
    expect(applied).toContain('-fflags +bitexact');
  });
...
describe('stripContainerMetadata', () => {
  it('remuxes with both streams copied and the tags dropped', async () => {
    queue({ code: 0 });

    const res = await audio.stripContainerMetadata('raw.mp4', 'out.mp4');

    expect(res).toEqual({ stripped: true });
    const args = spawn.mock.calls[0][1].join(' ');
    expect(args).toContain('-c copy');
    expect(args).toContain('-map_metadata -1');
    expect(args).toContain('-fflags +bitexact');
    expect(args).not.toMatch(/loudnorm/);
  });

  it('reports a missing ffmpeg and leaves no half-written output', async () => {
    queue({ error: true });
