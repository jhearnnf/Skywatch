...
export default function BOOBriefsList() {
  ...
  // Returns: 'no-data' | 'needs-read' | 'needs-quiz' | 'active'
  ...
  const filtered = briefs
    .filter(b => {
      const s = getState(b)
      if (activeTab === 'Available') return s === 'active' || s === 'needs-quiz' || s === 'needs-read'
    ...
    .sort((a, b) => {
      const ORDER = { 'active': 0, 'needs-quiz': 1, 'needs-read': 2, 'no-data': 3 }
