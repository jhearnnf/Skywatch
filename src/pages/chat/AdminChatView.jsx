    ...
    <div className="h-[calc(100dvh-11rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] flex md:gap-3">
      <div className={`w-full md:w-72 md:shrink-0 ${selectedId ? 'hidden md:flex' : 'flex'} flex-col bg-surface rounded-2xl border border-slate-200 card-shadow overflow-hidden`}>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {conversations.map(c => {
            // A resolved ticket stays in the list, so it has to read as done
            // at a glance: dimmed, struck through, with a tick.
            const resolved = c.type === 'support' && c.status === 'closed'
            return (
            <button
              key={c._id}
              onClick={() => setSelectedId(c._id)}
              data-resolved={resolved || undefined}
              className={`w-full text-left px-3 py-2 border-b border-slate-100 transition-colors ${selectedId === c._id ? 'bg-brand-100' : 'hover:bg-slate-100'} ${resolved && selectedId !== c._id ? 'opacity-60' : ''}`}
            >
              <div className="flex items-center justify-between gap-2">
                <p className={`text-xs font-semibold truncate ${resolved ? 'text-slate-500 line-through' : 'text-slate-700'}`}>{rowLabel(c)}</p>
                {resolved && (
                  <span className="text-emerald-700 text-xs font-bold shrink-0" aria-label="Resolved" title="Resolved">✓</span>
                )}
