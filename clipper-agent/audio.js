...
// Ship a container that says nothing about how it was made. Remotion writes
// `comment=Made with Remotion x.y.z` into every MP4 and ffmpeg adds its own
// `encoder=Lavf...`; together they are a plain-text "generated by software"
// stamp in the one place a platform's upload pipeline reads first. The first
// Clipper video posted to TikTok got 5 views against a manual baseline of 200+,
// and while nobody outside TikTok knows what their classifier weighs, there is
// no reason to hand it this. -map_metadata -1 drops the global tags; bitexact
// stops the muxer writing a fresh encoder tag in their place.
const STRIP_METADATA_ARGS = ['-map_metadata', '-1', '-fflags', '+bitexact'];
...
// Metadata-only rewrite for the path where loudness matching was skipped, so
// the render never ships stamped just because its level could not be read.
// Both streams are copied; this is a remux, not an encode.
async function stripContainerMetadata(inPath, outPath) {
  const res = await run('ffmpeg', [
    '-y', '-v', 'error',
    '-i', inPath,
    '-c', 'copy',
  ...
  if (!res.ok) {
    await fs.rm(outPath, { force: true });
    return { stripped: false, reason: res.missing ? 'ffmpeg not on PATH' : 'remux failed' };
  }
  return { stripped: true };
}

module.exports = {
  probeDurationMs, trimSilence, normaliseLoudness, stripContainerMetadata, parseLoudnormJson, trimFilter,
  TARGET, SILENCE_THRESHOLD, HEAD_PAD_SEC, TAIL_PAD_SEC, STRIP_METADATA_ARGS,
};
